...
public class Term {
	...
	public double evaluate(Vector assignments) throws PolyException {
           ...
           HashMap<String,Double> e = new HashMap<String,Double>();
            ...
            for (int i = 0; i<_vars.size(); i++) 
                e.put(_vars.get(i), assignments.getval(_vars.get(i))); // getval is get(key) for hashmap functions, _vars.get(i) gets the string we need to input into that
            
            HashMap<Double,Integer> eval = new HashMap<Double,Integer>();
            
            for (int i = 0; i<_vars.size(); i++) 
                eval.put(e.get(_vars.get(i)),(_pows.get(i))); // Makes the key value pair - the value of the variables and the value 
            
            double val;
            double prod=1;
            
            for (double key: eval.keySet()) 
                prod *= Math.pow((key),(eval.get(key)));
	...
	public Term differentiate(String var)  {
               ...
               // Var is taken in from the function
               // data members already exist for the function to act as the term
            int pow;  
            int nPow;// this will be the new power that the power gets convereted
            int index; // will be the index that string var is in
               
               Term d = new Term(1);//assign it to values of term this function is called upon
               d._coef=this._coef;
               
               for (int i = 0; i<this._vars.size(); i++)
                    d._vars.add(this._vars.get(i));
               
               for (int j = 0; j<this._pows.size();j++)
                    d._pows.add(this._pows.get(j));
               
               if(d._vars.contains(var)!=true){
                d._coef =0;
                return d;
             }
               
              index = d._vars.indexOf(var);
              pow = d._pows.get(index);
              nPow = pow - 1 ;
              
              if (nPow==0) {
                  d._vars.remove(index);
                  d._pows.remove(index);
              }
              else {
                d._coef*=pow;
                d._pows.set(index, nPow);
              }
